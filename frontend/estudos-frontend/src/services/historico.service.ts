...
export class HistoricoService {
  ...
  carregarHistorico(usuarioId: number) {
    this.http
      .get<any[]>(`${this.apiUrl}?usuarioId=${usuarioId}`)
      .subscribe((resp) => {
          ...
          const sessoes: HistoricoItem[] = resp[0].Sessoes.map((s: any) => ({
            ...
            .sort(
              (a: HistoricoItem, b: HistoricoItem) =>
                new Date(b.encerradaEm).getTime() -
                new Date(a.encerradaEm).getTime(),
            );
      ...
      .subscribe((resp) => {
        ...
        if (resp.length > 0) {
          ...
          const totalMinutos = novasSessoes.reduce(
            (acc: number, s: any) => acc + s.duracaoDaSessao,
            0,
          );

          this.http
            .patch(`${this.apiUrl}/${historico.id}`, {
          ...
          this.http
            .post(this.apiUrl, novoHistorico)
            .subscribe(() => this.carregarHistorico(usuarioId));
        }
      });
  }
